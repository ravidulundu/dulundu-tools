...
const detectContent = (text: string) => {
    ...
    if ((trimmed.startsWith('{') || trimmed.startsWith('[')) && (trimmed.endsWith('}') || trimmed.endsWith(']'))) {
        try { JSON.parse(trimmed); return { type: 'json', label: 'Format JSON', toolId: 'json-formatter' }; } catch (e) { }
    }
    if (/^\d{10}(\d{3})?$/.test(trimmed)) return { type: 'date', label: 'Convert Timestamp', toolId: 'date-converter' };
    if (trimmed.length > 20 && /^[A-Za-z0-9+/]*={0,2}$/.test(trimmed) && trimmed.length % 4 === 0) {
        try { if (/[\x20-\x7E]/.test(atob(trimmed))) return { type: 'base64', label: 'Decode Base64', toolId: 'base64-converter' }; } catch (e) { }
    }
    return null;
};
...
const Popup = () => {
    ...
    const [smartAction, setSmartAction] = useState<{ type: string, label: string, toolId: string } | null>(null);
    ...
    const recentTools = useMemo(() => ALL_TOOLS.filter(t => recents.includes(t.id)).sort((a, b) => recents.indexOf(a.id) - recents.indexOf(b.id)), [recents]);
    const favoriteTools = useMemo(() => ALL_TOOLS.filter(t => favorites.includes(t.id)), [favorites]);
        ...
        <div className="w-[400px] h-[550px] bg-slate-50 flex flex-col font-sans text-slate-900">
                    ...
                    <div
                        onClick={async () => {
                            ...
                            if (tool) {
                                ...
                                } catch (e) { handleToolClick(tool.path, tool.id); }
                            }
                        }}
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-4">
                            ...
                            <div className="col-span-2 space-y-1">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {favoriteTools.map(tool => (
                                        <button key={tool.id} onClick={() => handleToolClick(tool.path, tool.id)} className="flex items-center gap-2 p-2 bg-white border border-gray-100 rounded-lg hover:border-primary/30 hover:shadow-sm transition-all text-left">
                                            <div className="p-1.5 bg-yellow-50 text-yellow-600 rounded"><tool.icon size={14} /></div>
                            ...
                            <div className="col-span-2 space-y-1 mt-2">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {recentTools.map(tool => (
                                        <button key={tool.id} onClick={() => handleToolClick(tool.path, tool.id)} className="flex items-center gap-2 p-2 bg-white border border-gray-100 rounded-lg hover:border-primary/30 hover:shadow-sm transition-all text-left">
                                            <div className="p-1.5 bg-slate-50 text-slate-500 rounded"><tool.icon size={14} /></div>
                                            <span className="text-xs font-medium truncate">{tool.name}</span>
                                        </button>
                                    ))}
                                </div>
                ...
                <div className="space-y-1">
                    {!search && <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider px-2 mb-1">All Tools</h4>}
                        ...
                        filteredTools.map((tool) => (
                            ...
                            >
                                ...
                                <div
                                    ...
                                    className={`p-1.5 rounded-full hover:bg-slate-100 transition-colors ${favorites.includes(tool.id) ? 'text-yellow-400' : 'text-slate-300 opacity-0 group-hover:opacity-100'}`}
                                >
