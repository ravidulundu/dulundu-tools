      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 flex flex-col h-full overflow-hidden">
        ...
        <div className="p-4 md:p-6 border-b border-gray-100 flex items-center justify-between bg-slate-50/50">
           <div className="flex items-center space-x-3">
             ...
             <div>
                <h1 className="text-xl md:text-2xl font-bold text-slate-800">TSV Converter</h1>
                <p className="text-xs md:text-sm text-slate-500">Convert Tab Separated Values to JSON or CSV</p>
             </div>
           </div>
        ...
        <div className="p-3 bg-white border-b border-gray-100 flex justify-center gap-4">
            <button 
               ...
               className={`px-4 py-2 rounded-lg text-sm font-medium transition-all border ${mode === 'tsv-json' ? 'bg-primary text-white border-primary' : 'bg-white text-slate-600 border-gray-200 hover:border-primary'}`}
             >
                TSV to JSON
             </button>
             <button 
               onClick={() => setMode('tsv-csv')}
               className={`px-4 py-2 rounded-lg text-sm font-medium transition-all border ${mode === 'tsv-csv' ? 'bg-primary text-white border-primary' : 'bg-white text-slate-600 border-gray-200 hover:border-primary'}`}
             >
                TSV to CSV
             </button>
        </div>
        ...
        <div className="flex-1 p-4 md:p-6 overflow-hidden bg-gray-50/30">
          <div className="grid md:grid-cols-2 gap-4 h-full">
            ...
            <div className="flex flex-col h-full min-h-[300px]">
              <label className="block text-sm font-bold text-slate-700 mb-2">Input TSV</label>
