         ...
         <div className="bg-white rounded-2xl shadow-sm border border-gray-200 flex flex-col h-full overflow-hidden">
            ...
            <div className="p-3 bg-white border-b border-gray-100 flex flex-col gap-3">
               <div className="flex items-center gap-2">
                  ...
                  >
                     {loading ? <RefreshCw className="animate-spin mr-1.5" size={16} /> : <Search className="mr-1.5" size={16} />}
                  ...
                  <div className="h-full grid md:grid-cols-2 gap-4">
                     <div className="flex flex-col gap-4 overflow-y-auto pr-2 custom-scrollbar">
                        <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                           <h3 className="font-bold text-slate-700 mb-4 border-b border-gray-100 pb-2 text-sm uppercase tracking-wide">Status</h3>
                           <ul className="space-y-3 text-sm">
                              <li className="flex justify-between items-center"><span className="text-slate-500">Handle</span> <span className="font-mono bg-slate-50 px-2 py-0.5 rounded border border-gray-100">{data.handle}</span></li>
                              <li className="flex justify-between items-center"><span className="text-slate-500">Name</span> <span className="font-mono font-bold text-primary">{data.ldhName}</span></li>
                              <li className="flex flex-col mt-2 gap-2">
                                 <span className="text-slate-500">Status Flags</span>
                                 <div className="flex flex-wrap gap-1.5">
                                    {data.status?.map((s: string) => (
                                       <span key={s} className="px-2 py-1 bg-blue-50 text-blue-700 rounded-md text-xs border border-blue-100 font-medium">{s}</span>
                                    ))}
                                 </div>
                              </li>
                           </ul>
                        ...
                        <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
                           <h3 className="font-bold text-slate-700 mb-4 border-b border-gray-100 pb-2 text-sm uppercase tracking-wide">Events</h3>
                           <ul className="space-y-3 text-sm">
                              {data.events?.map((e: any) => (
                                 <li key={e.eventAction} className="flex justify-between items-center group hover:bg-slate-50 p-1.5 rounded-lg transition-colors -mx-1.5 px-1.5">
                                    <span className="text-slate-500 capitalize font-medium">{e.eventAction.replace(' last', '')}</span>
                                    <span className="font-mono text-slate-700">{new Date(e.eventDate).toLocaleDateString()}</span>
                                 </li>
                              ))}
                           </ul>
                        </div>
