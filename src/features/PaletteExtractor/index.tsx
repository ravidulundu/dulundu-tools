         ...
         <div className="bg-white rounded-2xl shadow-sm border border-gray-200 flex flex-col h-full overflow-hidden">
            ...
            <div className="flex-1 p-4 md:p-6 overflow-y-auto bg-gray-50/30 flex flex-col items-center">
               <div className="max-w-4xl w-full">
                  ...
                  >
                     ...
                     {image ? (
                        <div className="flex flex-col items-center">
                           <img src={image} alt="Preview" className="max-h-64 rounded-lg shadow-md mb-4 object-contain" />
                  ...
                  {loading && <div className="text-center text-slate-500 font-medium animate-pulse">Analyzing colors...</div>}
...
const ColorCard: React.FC<{ hex: string }> = ({ hex }) => {
   ...
   return (
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-md transition-shadow cursor-pointer group" onClick={handleCopy}>
         <div className="h-24 w-full relative" style={{ backgroundColor: hex }}>
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/10">
               <span className="bg-white/90 text-slate-800 text-xs font-bold px-2 py-1 rounded shadow-sm">Click to Copy</span>
            </div>
         </div>
         <div className="p-3 flex justify-between items-center">
            <span className="font-mono font-bold text-slate-700 uppercase">{hex}</span>
            {copied ? <Check size={16} className="text-green-500" /> : <Copy size={16} className="text-slate-400 group-hover:text-primary transition-colors" />}
         </div>
      </div>
   );
};
